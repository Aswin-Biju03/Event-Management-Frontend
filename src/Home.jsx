...
function Home() {
  ...
  useEffect(() => {
    ...
    getAllEventsAPI()
      ...
      .catch((err) => console.error("Error loading homepage events:", err));
  }, []);
  ...
  return (
    <div className="bg-bg text-text min-h-screen pt-16">
      ...
      <div className="bg-gradient-to-br from-surface to-black px-6 py-20">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              to="/events"
              className="bg-primary text-black px-7 py-3 rounded-xl font-medium hover:bg-primary-soft transition"
            >
              Browse Events
            </Link>
          </div>
          ...
          <div className="flex flex-wrap gap-10 mt-12 pt-8 border-t border-border">
            {[
              ["120+", "Events this year"],
              ["4,800+", "Tickets issued"],
              ["98%", "Happy attendees"],
            ].map(([number, label]) => (
              <div key={label}>
      ...
      {/* UPCOMING EVENTS CONTAINER GRID */}
      <div className="max-w-6xl mx-auto px-6 py-12">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold">Upcoming Events</h2>
          <Link to="/events" className="text-muted hover:text-text text-sm">
            View all →
          </Link>
        </div>
