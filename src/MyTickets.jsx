...
export default function MyTickets() {
  ...
  useEffect(() => {
    ...
    getUserBookingsAPI(user._id)
      ...
      .catch((err) => {
        console.error(err);
        setError("Failed to load digital gate passes.");
        setLoading(false);
      });
  }, []);

  if (loading) return <div className="text-white text-center pt-24">Loading active passes...</div>;
  ...
  return (
    <div className="w-full min-h-screen bg-bg text-text px-6 pt-24 pb-12">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-sm text-muted mb-8">Present these QR codes at the entryway check-in checkpoint.</p>
          ...
          {tickets.map((t) => {
            ...
            return (
              <div key={t._id} className="bg-surface border border-border rounded-2xl p-5 flex items-center gap-5 shadow-lg">
                ...
                {/* QR VALUE IS INJECTED DIRECTLY WITH THE STRIPPED STRING FORMAT UUID */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-4">
                    <span className={`text-xs px-2.5 py-1 rounded-full border font-semibold ${
                      t.attended ? "bg-green-500/10 border-green-500 text-green-400" : "bg-primary/10 border-primary text-primary"
                    }`}>
